...
class NewsToolbar(CMSToolbar):
    ...
    def populate(self):
        ...
        newsitem_menu = admin_menu.get_or_create_menu('news-menu', _('Research ...'), position=position)
        ...
        newsitem_menu.add_sideframe_item(_('Research Category List'), url=url)

        url = reverse('admin:news_newscategory_add')
        newsitem_menu.add_modal_item(_('Add New Research Category'), url=url)

        newsitem_menu.add_break()

        url = reverse('admin:news_newsitem_changelist')
        newsitem_menu.add_sideframe_item(_('Research Item List'), url=url)

        url = reverse('admin:news_newsitem_add')
        newsitem_menu.add_modal_item(_('Add New Research Item'), url=url)
